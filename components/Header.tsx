...
export function Header({ userSlot }: { userSlot?: ReactNode }) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-gradient-to-b from-[#050403]/88 to-transparent pb-2 pt-3 sm:px-6 sm:pb-3 sm:pt-4">
      <nav
        aria-label="Navigation principale"
        className="glass-nav mx-auto flex min-h-14 max-w-7xl items-center justify-between gap-3 rounded-full px-3 py-2 sm:min-h-16 sm:gap-4 sm:px-5 sm:py-2.5"
      >
        <div className="flex min-w-0 flex-1 items-center gap-3">
          <Link
            href={logoHref}
            className="shrink-0 font-display text-lg leading-none text-cream outline-none transition hover:text-champagne glass-focus-ring sm:text-2xl"
            aria-label={
              isHome
        ...
        {!isPrivateRoute ? (
          <div className="hidden min-w-0 flex-1 items-center justify-center gap-6 text-sm text-[#dbcdb8] md:flex lg:gap-8">
            <Link
              className="transition hover:text-cream glass-focus-ring"
              href={experienceHref}
            >
              Expérience
            </Link>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          {isDemoRoute ? (
            <>
              ...
              <Link
                href="/"
                className="hidden text-xs font-medium text-champagne/90 transition hover:text-cream glass-focus-ring min-[390px]:inline sm:text-sm"
              >
                Accueil Vistaire
              </Link>
            </>
            ...
            <>
              <Link
                href="/demo"
                className="text-xs font-medium text-champagne/90 transition hover:text-cream glass-focus-ring sm:text-sm"
              >
                Menu client
              </Link>
              <Link
                href="/"
                className="hidden text-xs font-medium text-[#cdbfa9] transition hover:text-cream glass-focus-ring min-[390px]:inline sm:text-sm"
              >
                Accueil Vistaire
              </Link>
            </>
            ...
            <>
              ...
              <Link
                href="/demo"
                className="text-xs font-medium text-[#cdbfa9] transition hover:text-cream glass-focus-ring sm:text-sm"
              >
                Menu exemple
              </Link>
            </>
